...
Estrategias:
- search: Busqueda por query
- stargazers: Usuarios que dieron estrella a un repo
- followers: Seguidores de un usuario
- trending: Owners de repos con muchas estrellas
"""
...
class GitHubDiscovery(GitHubFollowerManager):
    ...
    def discover_by_search(self, query):
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Encontrados {len(items)} usuarios en la busqueda.")
            return [user['login'] for user in items]
        else:
            logger.error(f"Error en busqueda: {response.status_code} - {response.text}")
            return []

    def discover_by_repo_stargazers(self, repo):
    ...
    def is_likely_to_follow_back(self, user_info):
        ...
        if followers < 10:
            ...
            return True
        if following == 0:
            logger.info(f"Saltando @{username} (no sigue a nadie)")
            return False
        ...
        if ratio >= self.min_follow_ratio:
            ...
            logger.info(f"Saltando @{username} (ratio bajo: {ratio:.2f})")
            return False

    def run_discovery(self, strategy, target):
        ...
        if not usernames:
            logger.warning("No se encontraron usuarios con esta estrategia.")
            return
        ...
        if not candidates:
            logger.info("Ya sigues a todos los candidatos encontrados.")
            return

        followed_count = 0
        for username in candidates:
            if followed_count >= self.max_discovery_follows:
                logger.info(f"Limite de {self.max_discovery_follows} follows alcanzado.")
                break
