...
- Seguir a usuarios que te siguen pero tu no los sigues
- Lista blanca (whitelist) para proteger usuarios especificos
...
class GitHubFollowerManager:
    ...
    def get_followers(self, username=None):
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"   Pagina {page}: {len(page_followers)} seguidores")
                page += 1
                time.sleep(0.5)
            else:
    ...
    def get_following(self, username=None):
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"   Pagina {page}: {len(page_following)} seguidos")
                page += 1
                time.sleep(0.5)
            else:
    ...
    def follow_user(self, username):
        if self.dry_run:
            logger.info(f"[DRY-RUN] Seguiria a @{username}")
            return True
    ...
    def unfollow_user(self, username):
        if self.dry_run:
            logger.info(f"[DRY-RUN] Dejaria de seguir a @{username}")
            return True
        url = f"{self.base_url}/user/following/{username}"
    ...
    def should_skip_user(self, user_data):
        ...
        if username in self.whitelist:
            logger.info(f"Saltando @{username} (en whitelist)")
            return True
        if user_data.get('followers', 0) > 10000:
            logger.info(f"Saltando @{username} (usuario popular)")
            return True
        if user_data.get('type') == 'Organization' and 'github' in username:
            logger.info(f"Saltando @{username} (organizacion GitHub)")
            return True
        return False
    ...
    def get_statistics(self):
        ...
        logger.info(f"Estadisticas: {stats['followers']} seguidores | {stats['following']} siguiendo | Ratio: {stats['ratio']:.2f}")
        return stats

    def run(self, action='both'):
        if self.dry_run:
            logger.info("MODO DRY-RUN ACTIVADO: No se realizaran cambios reales.")
